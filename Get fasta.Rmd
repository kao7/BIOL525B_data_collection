...
#Get genes
setwd("C:/Users/User/Desktop/Phylogeny course/HWK/Get fasta seq")
IDs <- scan("diverse_proteomes.txt", character())
url_list <- tibble(ID = IDs,
                   URL = rep("https://www.ebi.ac.uk/interpro/api/protein/UniProt/entry/InterPro/IPR008974/proteome/uniprot/REPLACE/?page_size=300",length(IDs))) %>% #"https://www.ebi.ac.uk/interpro/api/protein/UniProt/entry/InterPro/IPR008974/taxonomy/uniprot/REPLACE/?page_size=300"
    mutate(URL = str_replace(URL, "REPLACE", ID))


res_ <- map(url_list$URL, GET)
res_ <- discard(res_, ~.x$status_code == 204) #remove lists that don't have any data
data <- map(res_, "content") %>%
  ...
  as.character()%>%
  as_tibble() %>%
  distinct() # generates list of uniprot accessions containing interpro accessions - NOTE THAT SOME ISOFORMS MAY NOT BE INCLUDED
...
url_list3 <- tibble(Accession = data$value, URL = rep("https://www.ebi.ac.uk/interpro/api/entry/InterPro/protein/UniProt/REPLACE?page_size=350",nrow(data))) %>%
