...
mappings <- rbind(up_2_genes, up_2_genes2, up_2_genes3)  %>%
  ...
  arrange(Gene_ID) %>%
  ungroup() %>%
  mutate(isoform_problem = "Yes") %>%
  dplyr::select(fasta_name, isoform_problem)
  
  distinct(Gene_ID)
saveRDS(mappings, "mappings.RDS")
mappings<- readRDS("mappings.RDS")

tree_annot <- left_join(pdl_fasta, mappings) %>%
...
pdl_fasta_gID <- left_join(pdl_fasta, gID_mappings, by = "uniprot_accession") %>%
  mutate(Gene_ID = str_replace(Gene_ID, "LOC_", ""))
...
df <- data.frame(Species = tree_annot$species_short,
                 Isoform_issues = tree_annot$isoform_problem)
rownames(df) <- tree_annot$tip


raxml_annot <- ggtree(raxml, layout = "circular") %<+% tree_annot +
  geom_tiplab(aes(label= fasta_name, color = simp_type), size = 2) 

raxml_annot_ <- gheatmap(raxml_annot, df[, "Species", drop = F], offset= 1, width = 0.03, colnames = FALSE)
raxml_annot_2 <-gheatmap(raxml_annot_, df[, "Isoform_issues", drop = F], offset=1.3, width = 0.03, colnames = FALSE)
...
ggsave("raxml_annotated.pdf", plot = raxml_annot_2,
       device = "pdf", 
       width = 1000, 
       height =1000, 
       units = "mm",
